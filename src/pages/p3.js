...
const Page3 = () => {
  ...
  const { data: jobs, isPending, error } = useFetch("http://jazeera-travels.herokuapp.com/api/jobs?populate=Tags,image")
  {jobs&&console.log(jobs.data)}
...
  			<label for="4">Last 30 Days</label>
    ...
    { jobs&&jobs.data.map(job=>	
      <Card key = {job.id} style={{borderRadius:'15px', marginBottom:'16px', marginTop:'10px', padding:'20px' , width:'100%'}}>
          <Card.Body class="flex">
            <div className='float-left h-24 w-24 ml-4 md:ml-8'>
            <img className='h-16 w-16 mt-1' src={`http://jazeera-travels.herokuapp.com/${job.attributes.image.data.attributes.formats.thumbnail.url}`} alt="image" />
            </div> 
            <div class='float-right pl-2 pr-4 w-full'>
            <img src={bookmark} class='float-right mb-0' />
            <p class='text-2xl text-left mt-2'>{job.attributes.jobTitle}</p>
